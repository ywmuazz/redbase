...
public:
    ...
    RC ForcePages();
    RC PrintIndex();
    RC PrintIndex(std::map<int,std::vector<RID>>& mp);
...
private:
    ...
    //use链表link
    RC LinkSlot(IX_NodeHeader* header, int preindex, int newindex);
    RC UnlinkSlot(IX_NodeHeader* nHeader, int preindex, int delindex);
    RC LinkFreeList(IX_NodeHeader* nHeader, int index);
    RC GetFreeSlot(IX_NodeHeader* nHeader, int& index);
